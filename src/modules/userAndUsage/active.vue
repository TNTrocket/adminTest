    ...
    <table-component :table.sync="tables[0]">
        ...
        <template slot="table">
            <table class="table table-striped">
                ...
                <tr>
                    <th>日期</th>
                    <th v-for="(key,val) in productListName">{{val}}</th>
                </tr>
                </thead>
                <tbody>
                <tr v-for="item in tables[0].renderData | pageFilter" v-if="productType==='企邮类'">
                    <th scope="row">{{item.date}}</th>
                    <td>{{item["1"]}}</td>
                    <td>{{item["2"]}}</td>
                    <td>{{item["3"]}}</td>
                </tr>
                <tr v-for="item in tables[0].renderData | pageFilter" v-if="productType==='增值类'">
                    <th scope="row">{{item.date}}</th>
                    <td>{{item["4"]}}</td>
                </tr>
                <tr v-for="item in tables[0].renderData | pageFilter" v-if="productType==='客户端'">
                    <th scope="row">{{item.date}}</th>
                    <td>{{item["5"]}}</td>
                    <td>{{item["6"]}}</td>
                </tr>
    ...
    export default {
        ...
        watch: {
            ...
            productType(val,oval){
                this.getData(val,this.productIds[val]);
            }
        },
        computed:{
            productListName(){
        ...
        methods: {
                ...
                callApi({
                    ...
                    if (name === "企邮类") {
                        ...
                        for (let a in dataOrderByDate) {
                            renderData.push({
                                date: a,
                                "1": dataOrderByDate[a]["1"] || 0,
                                "2": dataOrderByDate[a]["2"] || 0,
                                "3": dataOrderByDate[a]["3"] || 0
                            })
                        }
                    }
                    else if (name === "增值类") {
                        ...
                        for (let a in dataOrderByDate) {
                            renderData.push({
                                date: a,
                                "5": dataOrderByDate[a]["5"] || 0,
                                "6": dataOrderByDate[a]["6"] || 0
                            })
                        }
                    }
                    ...
                    this.renderChart(renderChartData.slice(0).reverse(),name)
                },res=>{
                    bootbox.alert("请求失败")
                });
            ...
            renderChart(data,type) {
                let productTypeList=[];
                    for (let d in this.echartsList[type]){
                        productTypeList.push({
                            name: this.echartsList[type][d],
                            type: 'line',
                            data: data.map(obj => obj[d])
                        })
                    }
                let option = {
                    ...echartsConfig,
                    legend: {
                        data: this.echartsArrList[type]
                    },
                    xAxis: [
                        {
                            type: 'category',
